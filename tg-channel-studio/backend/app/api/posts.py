...
async def _get(session: AsyncSession, post_id: int) -> Post:
...
async def list_posts(
    ...
    return [PostOut.model_validate(p) for p in (await session.scalars(stmt)).all()]


@router.post("/generate", response_model=PostOut, status_code=201)
async def generate(body: GenerateRequest, session: AsyncSession = Depends(get_session)) -> PostOut:
    """Manual generation: rewrite a specific raw post, or from scratch by channel topic."""
...
async def update_post(
    post_id: int, body: PostUpdate, session: AsyncSession = Depends(get_session)
) -> PostOut:
...
async def approve_post(post_id: int, session: AsyncSession = Depends(get_session)) -> PostOut:
    """Move a review/draft/failed post into the publishing schedule."""
    post = await _get(session, post_id)
...
async def publish_now(post_id: int, session: AsyncSession = Depends(get_session)) -> PostOut:
    post = await _get(session, post_id)
    if post.status == PostStatus.published:
...
async def reject_post(post_id: int, session: AsyncSession = Depends(get_session)) -> PostOut:
    post = await _get(session, post_id)
    if post.status == PostStatus.published:
...
async def delete_post(post_id: int, session: AsyncSession = Depends(get_session)) -> None:
    post = await _get(session, post_id)
